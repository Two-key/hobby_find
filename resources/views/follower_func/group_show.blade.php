    ...
    <body>
        ...
        <h2 class="pt-0">
            <div class="bg-contain"
                style="background-image: url('https://res.cloudinary.com/dpbph7hyn/image/upload/v1693746859/Untitled_design_1_tnbdkx.svg'); height: 250px">
            </div>
        </h2>
        <div class="absolute bottom-0 left-0 right-0 top-10 h-fit w-full overflow-hidden bg-fixed"
            ...
            <main class="py-10 text-blue-950">
                ...
                <h1
                    class="text-indigo-950 flex justify-center text-4xl font-bold underline decoration-yellow-400 mt-10">{{ $category->category_name }}の船
                </h1>
                <div class="flex basis-auto space-x-5 ml-5 pt-12">
                    @foreach ($groups as $group)
                    <a href="/group_show/{{ $group->id }}">
                        <div
                            class="block max-w-xs rounded-lg bg-white shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)] dark:bg-neutral-700">
                            ...
                            <div class="p-6">
                                <p
                                    class="flex justify-center font-bold text-base text-neutral-600 dark:text-neutral-200">
                                    {{ $group->title }}
                                </p>
                            </div>
                        </div>
