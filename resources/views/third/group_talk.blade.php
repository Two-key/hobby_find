        ...
        <footer class="">
            <form action="/{{$group-> id}}/messages" method="POST">
                ...
                <div class="w-max pt-4">
                    <input type="text" name="message[username]" placeholder="名前を入れてください…" value="{{ old('message.username') }}" class="py-2 h-14"/>
                    <p class="username__error" style="color:red">{{ $errors->first('message.username') }}</p>
                </div>
                <div class="w-max pt-4">
                    <input type="text" name="message[message]" placeholder="入力してください…" value="{{ old('message.message') }}" class="py-2 h-14"/>
                    <p class="comment__error" style="color:red">{{ $errors->first('message.message') }}</p>
                </div>
                <div class="pt-4">
                    <button type="submit" class= "bg-indigo-950 hover:bg-indigo-700 text-yellow-400 rounded w-24 h-14">送信</button>
                </div>
                </div>
            </form>
        </footer>
